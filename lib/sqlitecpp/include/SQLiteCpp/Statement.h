...
public:
    ...
    Statement& operator <<( const int aValue );
    Statement& operator <<( const unsigned aValue );
    Statement& operator <<( const long long aValue );
    Statement& operator <<( const double aValue );
    Statement& operator <<( const std::string& aValue );
    Statement& operator <<( const char* aValue );
...
private:
    ...
    int						mLastBoundIndex;//!< Last index used by this->bind or 0 if nothing has been bound.
};
